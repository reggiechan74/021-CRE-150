...
def rank_properties(
    ...
    if direction == 'higher_is_better':
        ...
        # closer_to_subject: rank by distance from subject's value (subject is last in properties)
        subject = properties[-1]
        subject_val = subject.get(variable)
        if variable == 'condition' and isinstance(subject_val, str):
            subject_val = CONDITION_ENCODING.get(subject_val.lower(), 4)
        if isinstance(subject_val, bool):
            subject_val = 1 if subject_val else 0
        if subject_val is not None:
            sorted_values = sorted(values, key=lambda x: abs(x[1] - float(subject_val)))
        else:
            sorted_values = sorted(values, key=lambda x: x[1])
